  ...
  tracksList.innerHTML = tracks
    .map((track, index) => {
      ...
      const playButton = track.previewUrl
        ? `<button class="play-btn hide-on-mobile ${
        ...
        : `<span class="play-btn disabled hide-on-mobile" title="Нема попереднього перегляду">-</span>`;

      const liked = isLiked(track.id);
      const likedClass = liked ? "is-liked" : "";
      const preferencesButton = track.previewUrl
        ? `<button class="add-to-preferences-btn hide-on-mobile ${
            liked ? "liked" : ""
          }" data-track-id="${track.id}" title="${
      ...
      const addToPlaylistBtn = track.previewUrl
        ? `<button class="add-to-playlist-btn hide-on-mobile" data-track-id="${track.id}" title="Додати в плейліст">
      ...
      return `
            <div class="track-item ${likedClass} ${isCurrentTrackPlaying ? 'playing' : ''}" data-track-id="${
        track.id
